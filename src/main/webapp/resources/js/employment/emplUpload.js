...
function exceltoJson(obj) {
	const selectedFile = obj.files[0]; // 선택된 파일 사용하기
	/*
		FileReader 객체는 웹 애플리케이션이 비동기적으로 데이터를 읽기 위하여 읽을 파일을 가리키는 File 혹은 Blob 객체를 이용해
		파일의 내용을(혹은 raw data버퍼로) 읽고 사용자의 컴퓨터에 저장하는 것을 가능하게 해줍니다.
	*/

	var reader = new FileReader(); // HTML5의 File API의 FileReader 객체 생성
	var arrayData = new Array(); // 엑셀 파일로부터 가져온 데이터를 Map에 담기위한 배열 객체 생성

	reader.onload = function(evt) {
		// console.log("evt : " + evt);
		// console.log("evt : " + evt.target.readyState);
		
		if(evt.target.readyState == FileReader.DONE) {
			var data = evt.target.result;
			data = new Uint8Array(data); // 처리할 바이너리가 메모리에서 비트 연산에 효율적으로 사용될 유형 선택.. 내공 쌓고 다시보기
			// call 'xlsx' to read the file
			var workbook = XLSX.read(data, {type: 'array'});
			// console.log("workbook : " + workbook);
			var arrRoa = XLSX.utils.sheet_to_row_object_array(workbook.Sheets['DataSheet'], {header:1}); // array를 json으로 변환

			arrRoa.forEach(function(rowObj, rowIdx) {
				if(rowIdx<2) return; // 엑셀에서 데이터가 시작되는 행 지정
//				console.log("["+rowIdx+"] : "+rowObj);
				var tempMap = {};

				// for(var i=0;i<e.length;i++) {
				// 	if(i==0) continue;
				// 	var v = isNull(e[i]) ? "★" : e[i];
				// 	console.log("i : "+i+" / "+cols[i-1][1]+" / "+v);
				// 	tempMap[cols[i-1][1]] = v;
				// }

				/* 컬럼값이 빈값(empty)이면 forEach에서 실행을 안시켜서 for문으로 대체함
				e.forEach(function(v, idx2) {
					console.log("idx2 : "+idx2+" / "+cols[idx2-1][1]+" / "+v);
					if(idx2==0) return;
					tempMap[cols[idx2-1][1]] = v;
				});
				*/

				cols.forEach(function(colObj, colIdx) {
					// ※ 엑셀에서 입력된 공백문자는 sheetjs에서 알아서 trim()이 되는듯..??

					// if(colIdx<1) return; // 엑셀이 아닌 그리드 정보에서 시작되는 열
					var v = rowObj[colIdx-1]; // 그리드(obj)와 엑셀(e) 간의 행 위치를 맞추기 위해서 인덱스 조정
					v = nullToEmpty(v); // null 이면 ""으로 치환
					
					// console.log("["+rowIdx+"]"+"["+colIdx+"]"+colObj[1]+" : "+v+" attr : " + colObj[2]);

					if(colObj[2] == "NUMBER") {
						v = __getNum(v);
					} else if(colObj[2] == "ENGNUM") {
						v = __getEngNum(v);
					} else if(colObj[2] == "ENG") {
						v = __getEng(v);
					}	
					tempMap[colObj[1]] = v;
				});
				arrayData.push(tempMap);

				// key.forEach(function(idx, value) {
				// 	for(var i=0;i<cols.length;i++) { tempMap[cols[i][0]] = key[i]; }
				// });

			});
			// console.log(arrayData);

			// 위 코드는 엑셀내의 데이터를 가져오면서 정제시키고,
			// 그리드에서 유효성 검사가 일부 진행됨, 중복체크는 따로..왜이렇게 했냐고(?첨해보니까 담엔 그리드에서 받을 땐 안해도 될듯..그때가서 또 연구해보겠지)
			__setGridData(arrayData, GRID1);
		}
	};
